...
test('정답(NH@fams2026!) + Enter → 프로젝트 관리 화면 진입', async ({ page }) => {
  await page.goto(FILE_URL);
  await dismissIntro(page);
  await page.click('#secret-ver');
  await page.fill('#pw-input', 'NH@fams2026!');
...
test('트리 메뉴 클릭 → 해당 섹션으로 이동 + 활성 표시', async ({ page }) => {
  ...
  await page.click('.pj-link-parent[data-pj="pj-flow"]');
  await expect(page.locator('.pj-link-parent[data-pj="pj-flow"]')).toHaveClass(/active/);
...
test('As-Is 하위 트리: 14개 화면 링크가 보이고, 클릭 시 해당 화면으로 이동', async ({ page }) => {
  ...
  // As-Is subgroup 클릭해서 펼침 (기본 접힘 상태)
  await page.click('.pj-link-parent[data-pj="pj-asis"]');
  await expect(page.locator('.pj-subgroup').first()).toHaveClass(/open/);
  // As-Is 화면 서브링크만 14개 (pjs-ps- 로 시작하는 것만)
  const asIsSubLinks = page.locator('.pj-sublink[data-pj^="pjs-ps"]');
...
test('AI 갤러리: #pj-ai 안에 .pj-shot이 4개 이상 존재 (MCP 4장 기준)', async ({ page }) => {
  await enterProject(page);
...
test('AI 갤러리: enterProjectMode 호출 후 MCP 첫 이미지 src가 주입됨', async ({ page }) => {
  await enterProject(page);
  // MCP 토픽(pjs-ai-platform-A) 안 첫 번째 이미지 확인
  const img = page.locator('#pjs-ai-platform-A .pj-shot-img').first();
  const src = await img.getAttribute('src');
  expect(src).toBeTruthy();
  expect(src).not.toBe('');
  expect(src).toContain('ai-mcp-00.png');
});
